import {
    ...
    domToUrlStructure,
    setHeaders,
    rebuildSandboxShortcode, getProjectIdFromUrl
} from '../helper.js';
...
jQuery('.project-builder section[data-id="2"] .save-changes').on('click', async function () {
    ...
    const urlMode = jQuery(`input:radio[name='mpg_url_mode_group']:checked`).val();
    ...
    Object.keys(dataObject).forEach(key => (dataObject[key] == null || dataObject[key] === undefined) && delete dataObject[key]); // Delete all null values
    
    window?.tiTrk?.with( 'multi' ).set( 'save-changes-2', {
        feature: 'dashboard-saved-changes',
        featureComponent: 'source-tab',
        value: dataObject,
        groupId: projectId
    } );
...
jQuery('#mpg_unschedule_task').on('click', async function () {
    ...
    if (decision) {
        ...
        setTimeout(() => {
            location.href = `${backendData.projectPage}&action=edit_project&id=${getProjectIdFromUrl()}`;
        }, 1000);
    }
})
...
jQuery('.main-template-info').on('submit', async function (e) {
    ...
    const projectIdValue = getProjectIdFromUrl();
    ...
    Object.keys(payload).forEach(key => (payload[key] == null || payload[key] === undefined) && delete payload[key]); // Delete all null values

    window?.tiTrk?.with( 'multi' ).set( 'save-changes-1', {
        feature: 'dashboard-saved-changes',
        featureComponent: 'template-tab',
        value: payload,
        groupId: projectIdValue || projectId
    } );
    window?.tiTrk?.uploadEvents();

    setTimeout(() => {
        window.location.href = backendData.projectPage + '&action=edit_project&id=' + projectId;
    }, 1000);
 ...
 */
document.querySelector('input[name="mpg_enable_telemetry"]')?.addEventListener('change', async function (event) {
...
document.querySelector('button.use-direct-link-button')?.addEventListener('click', () => {
...
document.querySelector('input[name="mpg_upload_file_input"]')?.addEventListener('click', () => {
    window.tiTrk?.with( 'multi' ).add({
        feature: 'dashboard',
        featureComponent: 'source-upload',
...
document.querySelector('[data-cache-type="disk"] button.enable-cache')?.addEventListener('click', (event) => {
...
document.querySelector('[data-cache-type="database"] button.enable-cache')?.addEventListener('click', (event) => {
    window.tiTrk?.with( 'multi' ).set( 'enable-disk-cache', {
        feature: 'dashboard',
